...
function ModelSelectorEffort({
  ...
  return (
    <div
      ...
      className={cn(
        "flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 border-t px-3 py-2",
        className,
      )}
      onKeyDown={(e) => {
        onKeyDown?.(e);
      ...
      >
        {efforts.map((option) => (
          <RadioGroupPrimitive.Item
            ...
            value={option.id}
            className={cn(
              "focus-visible:ring-ring/50 text-muted-foreground hover:text-foreground shrink-0 rounded-md px-1.5 py-1 text-xs transition-colors outline-none focus-visible:ring-2",
