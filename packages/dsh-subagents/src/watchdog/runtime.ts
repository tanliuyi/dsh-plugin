/**
 * Watchdog 运行时状态机（对齐上游 pi-subagents src/watchdog/runtime.ts）。
 *
 * dsh 事件映射：turn/start → handleTurnStart（= 上游 before_agent_start 的
 * 重置/基线），user/message(source.kind==='user') → handleUserPrompt（scope 与
 * delta 头部），assistant/message / tool/call / tool/result → 回合 delta 累积，
 * tool/result → handleToolResult（cadence），turn/end → handleAgentEnd（评审边界）。
 *
 * 平台差异：上游在 agent_end 同步等待评审完成（waitingAtAgentEnd 阻塞下一回合）；
 * dsh 的 turn/end 事件派发是同步的、无法阻塞循环，因此评审异步进行，若下一回合
 * 先开始，迟到警告按 lateWarningPolicy 标 stale 且不 auto-follow（与上游一致）。
 */

import { createHash } from 'node:crypto'
import { computeWatchdogRepoChangeSignature, type WatchdogRepoChangeSignature } from './change-signature.ts'
...
import { WatchdogTurnAccumulator } from './turn-delta.ts'
import {
...
type ContextLike = Pick<{ cwd?: string }, 'cwd'>
...
export class MainWatchdogRuntime {
  ...
  private turnAccumulator = new WatchdogTurnAccumulator()
  ...
  reset(_reason = 'reset', options: { clearReviewInputSignature?: boolean; resetChangeSignature?: boolean; clearLspLedger?: boolean; clearScope?: boolean; resetAutoFollow?: boolean } = {}): void {
    ...
    this.turnAccumulator = new WatchdogTurnAccumulator()
  ...
  /** turn/start：重置回合状态并刷新配置（= 上游 before_agent_start 的非 prompt 部分）。 */
  handleTurnStart(ctx: ContextLike): void {
    if (this.disposed) return
    // auto-follow 计数不在此重置：回合开始时尚不知道 incoming prompt 是否
    // 是本 watchdog 的 auto-follow（dsh 的 prompt 在 turn/start 之后才到达，
    // 经 user/message 事件；真实用户 prompt 在 handleUserPrompt 里重置）。
    this.reset('turn_start', {})
    this.refreshConfig(ctx.cwd ?? this.cwd)
    this.resetRepoChangeBaseline()
  }

  /** user/message（source.kind === 'user'）：记录 scope 并作为 delta 头部。 */
  handleUserPrompt(prompt: string, options: { autoFollow: boolean }): void {
    if (this.disposed) return
    this.userPrompt = prompt
    if (!options.autoFollow) {
      // 真实用户 prompt 重置 auto-follow 计数（对齐上游 before_agent_start 的
      // resetAutoFollow: !autoFollowPrompt）
      this.resetAutoFollowState()
      this.includeUserPromptInNextDelta = true
      if (prompt.trim()) this.scope.addPrompt(prompt)
    } else {
      this.includeUserPromptInNextDelta = false
    }
  }

  /** 插件来源的 user/message 是否命中排队中的 auto-follow prompt（命中即消费）。 */
  matchesPendingAutoFollow(prompt: string): boolean {
  ...
  /** assistant/message：回合 delta 累积。 */
  handleAssistantMessage(message: { content: unknown }): void {
    if (this.disposed) return
    this.turnAccumulator.addAssistantMessage(message)
  }

  /** tool/call：登记 callId → 工具名（供 tool/result 关联）。 */
  handleToolCall(callId: string, name: string): void {
    if (this.disposed) return
    this.turnAccumulator.registerToolCall(callId, name)
  }

  /** tool/result：累积 delta 段（cadence 由 handleToolResult 另行检查）。 */
  handleToolResultDelta(callId: string, message: { content: unknown }, error: unknown, meta: unknown): void {
    if (this.disposed) return
    this.turnAccumulator.addToolResult(callId, message, error, meta)
  }

  handleTurnEnd(_event: unknown, ctx: ContextLike): void {
    if (this.disposed) return
    this.refreshConfig(ctx.cwd ?? this.cwd)
    if (!this.isEnabled()) return
    // dsh：delta 由累积器在 turn 内逐步构建，这里把当前渲染入队（对齐上游
    // turn_end 缓冲语义；评审仍在 agent_end 边界触发）。
    this.includeUserPromptInNextDelta = false
    const delta = this.turnDeltaText()
    this.enqueueDelta(delta)
  }

  private turnDeltaText(): string {
    const sections: string[] = []
    if (this.includeUserPromptInNextDelta && this.userPrompt?.trim()) sections.push(['User prompt:', this.userPrompt].join('\n'))
    const rendered = this.turnAccumulator.render()
    if (rendered) sections.push(rendered)
    return sections.join(REVIEW_DELTA_SEPARATOR)
  }
  ...
  async handleAgentEnd(_event: unknown, ctx: ContextLike): Promise<void> {
    ...
    this.refreshConfig(ctx.cwd ?? this.cwd)
    if (!this.isEnabled()) return
    const changeSignature = this.resolveReviewChangeSignature(this.cwd)
