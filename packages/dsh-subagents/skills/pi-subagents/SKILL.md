...
Use this skill when the parent orchestrator needs one specialized child or composed orchestration. Use `workflowScript` in the `subagents` tool for all execution, including one isolated child. Use `return runs.run("main", { agent, task })` for one child and `runs.all([...])` for coordinated waves: sequence, parallelism, branching, retries, gate monitors, and aggregation. Scripted workflows start asynchronously by default; pass `async:false` only for a small foreground run.
