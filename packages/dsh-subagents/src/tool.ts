...
export function registerSubagentsTool(ctx: Context, deps: SubagentsDeps, sessionState: SessionState): () => void {
  return ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      action: {
        type: 'string',
        description: 'Management/control action: status, list, get, create, update, delete, eject, disable, enable, reset, refine, refine.show, refine.rollback, models, guide, doctor, interrupt, stop, steer, resume, children.list, grant-spawn-budget, mission.create/list/show/update/resolve-decision/attach-run/close, schedule.create/list/show/history/pause/resume/run/run-due/delete, watchdog.configure/recommend-model/status/check.',
      },
      ...
      toolTimeoutMs: { type: 'integer', description: 'Accepted for schema compatibility; dsh children are not hard-capped per tool call.' },
      turnBudget: { type: 'json', description: 'Accepted for schema compatibility; not enforced in dsh children.' },
      toolBudget: { type: 'json', description: 'Accepted for schema compatibility; not enforced in dsh children.' },
      usageBudget: { type: 'json', description: 'Accepted for schema compatibility; recorded per run when reported.' },
