...
function Select({ options, placeholder, className, "aria-label": ariaLabel, ...props }: SelectProps) {
  ...
  return (
    <SelectRoot {...props}>
      ...
      <SelectContent>
        {options.map(({ label, triggerLabel: _triggerLabel, disabled, textValue, ...itemProps }) => (
          <SelectItem
            key={itemProps.value}
            {...itemProps}
